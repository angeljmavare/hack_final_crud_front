...
function App() {
  ...
  const add = () => {
    ...
    Axios.post("http://3.90.107.7/create", {
      nombre: nombre,
      edad: edad,
      correo: correo,
    }).then(() => {
  ...
  const update = () => {
    Axios.put("http://3.90.107.7/update", {
      id: id,
      nombre: nombre,
      edad: edad,
      correo: correo,
  ...
  const borrarUsuario = (val) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Axios.delete(`http://3.90.107.7/delete/${val.id}`).then(() => {
          Swal.fire({
  ...
  const getUsuarios = () => {
    Axios.get("http://3.90.107.7/usuarios").then((response) => {
